#include "dot1d.cuh"

/*	Difference :
	Au lieux de directement deriver avec que des atomicAdd le
__shared__ noyau, on fait la méthode que j'avais avant
ou on fait une autre opération pour calc dx et dp.

	Mathématiquement ca correspond a deriver y=X@P+B
en dX=p@dY.T
	dx = (p @ ((y-_y)*dtanh(x@p)).T).T
	dp = x.T @ ((y-_y)*dtanh(x@p))
*/

#define BLOQUE 16

static __global__ void kerd_stricte_16__shared2(	//	2 version : 1 stricte et une non stricte
	uint X, uint Y,
	uint depart, uint T,
	float * x, float * y,
	float * p,
	float * locd)
{
	// <KERD(T, BLOQUE), KERD(Y,BLOQUE)>
	// <         BLOQUE,         BLOQUE>

	__shared__ float __partage__x[BLOQUE][BLOQUE];
	__shared__ float __partage__p[BLOQUE][BLOQUE];

	uint thx = threadIdx.x;
	uint thy = threadIdx.y;

	uint _y = thx + blockIdx.x * blockDim.x;
	uint _t = thy + blockIdx.y * blockDim.y;

	float s = 0;

	FOR(0, d, X/BLOQUE) {
		__partage__x[thy][thx] = x[(depart+_t)*( X ) + d*BLOQUE + thx];
		__partage__p[thy][thx] = p[_y*(X+1) + d*BLOQUE + thy];
		__syncthreads();

	//#pragma unroll
		FOR(0, i, BLOQUE) s += __partage__x[thy][i] * __partage__p[i][thx];
		__syncthreads();
	};

#define __partage__b __partage__x[0]

	if (thy == 0) __partage__b[thx] = p[_y*(X+1) + (X+1-1)];
	__syncthreads();

	float a = ACTIV((s + __partage__b[thx]));
	   y[(depart+_t)*Y + _y] = a;
	locd[(depart+_t)*Y + _y] = dACTIV(s,a);
};

void nvidia_dot1d_shared_2_16(	//	2 version : 1 stricte et une non stricte
	uint X, uint Y,
	uint depart, uint T,
	float * x, float * y,
	float * p,
	float * locd)
{
	if (T%BLOQUE!=0) ERR("ATTENTION T%%16 != 0 (T=%i)", T);
	if (X%BLOQUE==0 && Y%BLOQUE==0 && T%BLOQUE==0) {
		kerd_stricte_16__shared2<<<dim3(KERD(Y, BLOQUE), KERD(T, BLOQUE)), dim3(BLOQUE, BLOQUE)>>>(
			X, Y,
			depart, T,
			x, y,
			p,
			locd);
		ATTENDRE_CUDA();
	} else {
		nvidia_dot1d_naive(
			X, Y,
			depart, T,
			x, y,
			p,
			locd);
	}
}

//	============================= Derivation ==============================

/*static __global__ void kerd_stricte_deriv_2_16___tout_en_un(	//	2 version : 1 stricte et une non stricte
	uint X, uint Y,
	uint depart, uint T,
	float * x, float * y,
	float * p,
	float * locd,
	float * dy,
	float * dx,
	float * dp)
{
	// <KERD(T, BLOQUE), KERD(Y,BLOQUE)>
	// <         BLOQUE,         BLOQUE>

	__shared__ float __partage__x[BLOQUE][BLOQUE];
	__shared__ float __partage__p[BLOQUE][BLOQUE];

	__shared__ float __partage__l[BLOQUE][BLOQUE];

	uint thx = threadIdx.x;
	uint thy = threadIdx.y;

	uint _y = thx + blockIdx.x * blockDim.x;
	uint _t = thy + blockIdx.y * blockDim.y;

	float _locd = locd[(depart+_t)*Y + _y] * dy[(depart+_t)*Y + _y];

	__partage__l[thy][thx] = _locd;

	atomicAdd(&dp[_y*(X+1) + (X)], _locd);

	float _dp;// = 0;
	float _dx;
	FOR(0, d, X/BLOQUE) {
		__partage__x[thy][thx] = x[(depart+_t)*( X ) + d*BLOQUE + thx];
		__partage__p[thy][thx] = p[_y*(X+1) + d*BLOQUE + thy];
		_dp = 0;
		_dx = 0;
		__syncthreads();

		FOR(0, i, BLOQUE) {
			_dp += __partage__x[i][thy] * __partage__l[i][thx];//__partage__x[thy][i];
			_dx += __partage__p[thx][i] * __partage__l[thy][i];//__partage__p[i][thx];
		}
		__syncthreads();

		atomicAdd(&dx[(depart+_t)*( X ) + d*BLOQUE + thx], _dx); 
		atomicAdd(&dp[_y*(X+1) + d*BLOQUE + thy], _dp);
	};
};*/


static __global__ void kerd_stricte_16__shared2____dx(	//	2 version : 1 stricte et une non stricte
	uint X, uint Y,
	uint depart, uint T,
	float * x, float * y,
	float * p,
	float * locd,
	float * dy,
	float * dx,
	float * dp)
{

	//dx = (p @ ((y-_y)*dtanh(x@p)).T).T

	__shared__ float __partage__x[BLOQUE][BLOQUE];
	__shared__ float __partage__p[BLOQUE][BLOQUE];

	uint thx = threadIdx.x;
	uint thy = threadIdx.y;

	//uint _y = thx + blockIdx.x * blockDim.x;
	uint _x = thx + blockIdx.x * blockDim.x;
	uint _t = thy + blockIdx.y * blockDim.y;

	float s = 0;

	FOR(0, d, Y/BLOQUE) {
		__partage__x[thy][thx] = locd[(depart+_t)*Y+d*BLOQUE+thx] * dy[(depart+_t)*Y+d*BLOQUE+thx];//x[(depart+_t)*( X ) + d*BLOQUE + thx];
		__partage__p[thy][thx] = p[(d*BLOQUE+thy)*(X+1) + _x];//p[_y*(X+1) + d*BLOQUE + thy];
		__syncthreads();

	//#pragma unroll
		FOR(0, i, BLOQUE) s += __partage__x[thy][i] * __partage__p[i][thx];
		__syncthreads();
	};

	dx[(depart+_t)*X+_x]   = s;
};


static __global__ void kerd_stricte_16__shared2____dp(	//	2 version : 1 stricte et une non stricte
	uint X, uint Y,
	uint depart, uint T,
	float * x, float * y,
	float * p,
	float * locd,
	float * dy,
	float * dx,
	float * dp)
{

	//dp = x.T @ ((y-_y)*dtanh(x@p))

	__shared__ float __partage__x[BLOQUE][BLOQUE];
	__shared__ float __partage__p[BLOQUE][BLOQUE];

	uint thx = threadIdx.x;
	uint thy = threadIdx.y;

	uint _x = thx + blockIdx.x * blockDim.x;
	uint _y = thy + blockIdx.y * blockDim.y;

	float s = 0;
	float biais = 0;

	FOR(0, d, T/BLOQUE) {
		__partage__x[thy][thx] = locd[(depart+d*BLOQUE+thx)*Y+_y] * dy[(depart+d*BLOQUE+thx)*Y+_y];//x[(depart+_t)*( X ) + d*BLOQUE + thx];
		__partage__p[thy][thx] = x[(depart+(d*BLOQUE+thy))*X+_x];//p[_y*(X+1) + d*BLOQUE + thy];
		__syncthreads();

	#pragma unroll
		FOR(0, i, BLOQUE) {
			s += __partage__x[thy][i] * __partage__p[i][thx];
			if (_x == 0) biais += __partage__x[thy][i];
		}
		__syncthreads();
	};

#define __partage__b __partage__x[0]

	//if (thy == 0) __partage__b[thx] = p[_y*(X+1) + (X+1-1)];
	if (_x == 0) dp[_y*(X+1) + (X+1-1)] = biais;
	__syncthreads();

	dp[_y*(X+1)+_x] = s;
};

void d_nvidia_dot1d_shared_2_16(	//	2 version : 1 stricte et une non stricte
	uint X, uint Y,
	uint depart, uint T,
	float * x, float * y,
	float * p,
	float * locd,
	float * dy,
	float * dx,
	float * dp)
{
	if (T%BLOQUE!=0) ERR("ATTENTION T%%16 != 0 (T=%i)", T);
	if (X%BLOQUE==0 && Y%BLOQUE==0 && T%BLOQUE==0) {
		/*kerd_stricte_deriv_2_16___tout_en_un<<<dim3(KERD(Y, BLOQUE), KERD(T, BLOQUE)), dim3(BLOQUE, BLOQUE)>>>(
			X, Y,
			depart, T,
			x, y,
			p,
			locd,
			dy,
			dx,
			dp);*/
		kerd_stricte_16__shared2____dx<<<dim3(KERD(X, BLOQUE), KERD(T, BLOQUE)), dim3(BLOQUE, BLOQUE)>>>(
			X, Y,
			depart, T,
			x, y,
			p,
			locd,
			dy,
			dx,
			dp);
		kerd_stricte_16__shared2____dp<<<dim3(KERD(X, BLOQUE), KERD(Y, BLOQUE)), dim3(BLOQUE, BLOQUE)>>>(
			X, Y,
			depart, T,
			x, y,
			p,
			locd,
			dy,
			dx,
			dp);
		ATTENDRE_CUDA();
	} else {
		d_nvidia_dot1d_naive(
			X, Y,
			depart, T,
			x, y,
			p,
			locd,
			dy,
			dx,
			dp);
	}
}