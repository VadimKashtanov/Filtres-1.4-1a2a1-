...
void nvidia_filtres_shared(	//	2 version : 1 stricte et une non stricte
	...
	float * y)
{
	ASSERT(f_par_bloque % BLOQUE_FB == 0);
	kerd_filtre_shared/*mdt1_filtre__kerd*/<<<dim3(KERD(T, BLOQUE_T), KERD(bloques, BLOQUE_B), KERD(f_par_bloque, BLOQUE_FB)), dim3(BLOQUE_T, BLOQUE_B, BLOQUE_FB)>>>(
		depart, T,
